...
from scorers import RankScorer, ClassScorer2, DistanceScorer, AdditionScorer, RegressionScorer, UCBScorer, Base
...
log  = f"./study1/outcomes/{experiment}_1.log"

scorers = [
   RankScorer(baser=Base("none"), exampler=DifferenceExample("square")),
   RankScorer(baser=Base("l1")  , exampler=DifferenceExample("abs")),
   RankScorer(baser=Base("sql2"), exampler=DifferenceExample("square")),
...
feedbacks = [
   DeviationFeedback("squared"),
   DeviationFeedback("squared"),
   DeviationFeedback("absolute"),
   DeviationFeedback("absolute"),
   DeviationFeedback("squared"),
   DeviationFeedback("absolute")
]
