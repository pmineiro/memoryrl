...
log        = None#f"./study1/outcomes/late_experiments3.log.gz"
...
if __name__ == '__main__':
   ...
   learners = [
      MemorizedLearner(epsilon, CMT(max_memories, router, RankScorer(power_t=0 ), c=40, d=1, alpha=.5)),
      MemorizedLearner(epsilon, CMT(max_memories, router, RankScorer(power_t=0 ), c=40, d=2, alpha=.5)),
      MemorizedLearner(epsilon, CMT(max_memories, router, RankScorer(power_t=0 ), c=40, d=3, alpha=.5)),
      VowpalEpsilonLearner(epsilon=epsilon, power_t=0)
   ]

   environments = Environments([LocalSyntheticSimulation(20, n_context_feats=3, n_actions=2, n_contexts=50)]).binary().shuffle(range(1))
   ...
   Experiment(environments, learners, environment_task=SimpleEnvironmentTask(), evaluation_task=FinalPrintEvaluationTask()).config(**config).evaluate(log).filter_fin().plot_learners(span=50)
