...
log        = None#"./study1/outcomes/full-openml-class-3.log.gz"
...
if __name__ == '__main__':
   ...
   learners = [
      ...
      #MemorizedLearner2(epsilon, CMT(6000, PCARouter(['x']), RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(100), v=(2,), d=0, alpha=0), "xxa", True, True),
      #MemorizedLearner2(epsilon, CMT(6000, PCARouter(['x']), RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(100), v=(2,), d=0, alpha=0), "xxa", True, False),

      #MemorizedLearner1(epsilon, CMT(6000, RPRouter(['x']), RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(100), v=(2,), d=0  , alpha=0)),
      #MemorizedLearner1(epsilon, CMT(6000, RPRouter(['x']) , RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(30), v=(2,), d=.1, alpha=0)),
      #MemorizedLearner1(epsilon, CMT(6000, RPRouter(['x']) , RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(30), v=(2,), d=.1, alpha=.1)),
      #MemorizedLearner1(epsilon, CMT(6000, RPRouter(['x']) , RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(30), v=(2,), d=.1, alpha=.25)),

      #MemorizedLearner1(epsilon, CMT(6000, None, DistScorer("exp",['x','a','xa']), c=NeverSplitter(), v=(2,), d=0.00, alpha=0)),
      MemorizedLearner1(epsilon, CMF(5, 6000, ProjRouter(['x'],"RNG",samples=250), RankScorer("exp",['x','a','xa']), c=LogSplitter   (20), v=(2,), d=0.00, alpha=0)),
      MemorizedLearner1(epsilon, CMT(   6000, ProjRouter(['x'],"PCA"), RankScorer("exp",['x','a','xa']), c=ConstSplitter(100), v=(2,), d=0.00, alpha=0)),
      MemorizedLearner1(epsilon, CMT(   6000, ProjRouter(['x'],"RNG",samples=250), RankScorer("exp",['x','a','xa']), c=ConstSplitter(100), v=(2,), d=0.00, alpha=0)),
      #MemorizedLearner1(epsilon, CMT(6000, RPRouter(['x']) , RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(30), v=(2,), d=.1, alpha=.5)),
      #MemorizedLearner1(epsilon, CMT(6000, RPRouter(['x']) , RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(30), v=(2,), d=.05, alpha=.5)),
      
      #MemorizedLearner1(epsilon, CMT(6000, RPRouter(['x']) , RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(30), v=(2,), d=.05, alpha=0)),
      
      #MemorizedLearner1(epsilon, CMT(6000, PCARouter(['x']), RankScorer2("exp",['x','a','xa'],True ), c=ConstSplitter(100), v=(2,), d=0, alpha=0)),
      #MemorizedLearner1(epsilon, CMT(6000, PCARouter(['x']), MetricScorer('exp',['x','a','xa']    ), c=ConstSplitter(100), v=(2,), d=0, alpha=0)),
      
      #MemorizedLearner2(epsilon, CMT(6000, PCARouter(['x']), RankScorer("exp",['x','a','xa'],True ), c=ConstSplitter(100), v=(2,), d=0, alpha=0), "xxa", False, False),
   ]
   ...
   #environments = Environments.from_openml(554, cat_as_str=True).filter(MNIST_LabelFilter(['9','4'])).filter(MNIST_SVD(30)).scale().shuffle([1,2,3,4]).take(500)
   ...
   #environments = Environments.from_openml(task_id=benchmark_tasks, cat_as_str=True, take=4000).scale()

   #seed = list(range(10))
   #seed = 1
   #environments = Environments.from_kernel_synthetic(2000, n_context_features=3, n_action_features=0 ,seed=seed).scale(shift='min',scale='minmax',target='rewards')
   
   Experiment(environments, learners, environment_task=SimpleEnvironmentTask(), evaluation_task=FinalPrintEvaluationTask()).config(**config).evaluate(log).filter_fin().plot_learners(sort="y")
