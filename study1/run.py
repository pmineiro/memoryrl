...
from scorers import RandomScorer, DistScorer, RankScorer, RankScorer4
...
if __name__ == '__main__':
   ...
   learners = [
      ...
      #MemorizedLearner1(epsilon, CMT(6000, None, RankScorer4("l1" ,['x','a','xa']), c=NeverSplitter(), d=0.00, alpha=0)),
   ...
   #environments = Environments.from_openml(139, cat_as_str=True, take=6000).scale().shuffle([100]).take(20)
   ...
   #environments = environments.scale().shuffle([1]).take(1000)
   ...
   environments = Environments.from_openml(task_id=benchmark_tasks, cat_as_str=True, take=10000).scale()

   #seed = list(range(4))
   #seed = 1
   #environments = Environments.from_kernel_synthetic(100, n_context_features=20, n_action_features=0 ,seed=seed).scale(shift='min',scale='minmax',target='rewards').binary()
   
   result = Experiment(environments, learners, environment_task=ClassEnvironmentTask(), evaluation_task=FinalPrintEvaluationTask()).config(**config).evaluate(log)
   result.filter_fin().plot_learners(y='rank',sort="y")
   result.filter_fin().plot_learners(y='reward',sort="y")
